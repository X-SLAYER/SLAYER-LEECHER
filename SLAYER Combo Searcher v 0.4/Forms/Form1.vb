...
#End Region
    ...
    Private Sub BTN1_Click(sender As Object, e As EventArgs) Handles BTN1.Click
        ...
        For j = 0 To Me.countThreads - 1
            Me.Thread(j) = New Thread(New ThreadStart(AddressOf Me.brute))
    ...
    Private Sub Bleech_RunWorkerCompleted(sender As Object, e As RunWorkerCompletedEventArgs)
        MsgBox("Done ^__^", MsgBoxStyle.Information, Environment.UserName)
    End Sub
    ...
    Private Sub BckYandex_DoWork(sender As Object, e As DoWorkEventArgs)
        Try
            Dim str As String
            For Each str In Me.TextBox1.Text.Split(New Char() {ChrW(13)})
                Try
                    Dim enumerator As IEnumerator
                    Dim enumerator2 As IEnumerator
                    Dim request As HttpWebRequest = DirectCast(WebRequest.Create(String.Concat(New String() {"https://yandex.ru/search/?text=", str, "&num=100&tbs=qdr:", Duréé, "&ei=-d2PUqnoIOyp7AbP2IGoBQ&start=", Conversions.ToString(22), "&sa=N&biw=1366&bih=604"})), HttpWebRequest)
                    Dim response As HttpWebResponse = DirectCast(request.GetResponse, HttpWebResponse)
                    Dim input As String = New StreamReader(response.GetResponseStream).ReadToEnd
                    Me.RchYandex.Text = input
                    Dim matchs As MatchCollection = New Regex("q=http://+(?:[a-zA-Z0-9./_+-]{3,})").Matches(Me.RchYandex.Text)
                    Dim builder As New StringBuilder
                    Try
                        enumerator = matchs.GetEnumerator
                        Do While enumerator.MoveNext
                    ...
                    End Try
                    Me.TextBox3.Text = (Me.TextBox3.Text & builder.ToString)
                    Dim matchs2 As MatchCollection = New Regex("q=https://+(?:[a-zA-Z0-9./_+-]{3,})").Matches(Me.RchYandex.Text)
                    Dim builder2 As New StringBuilder
                    Try
                        enumerator2 = matchs2.GetEnumerator
                        Do While enumerator2.MoveNext
...
#Region "LEECHING"
    ...
    Public Sub StopThreads()
        ...
        For i = 0 To Me.countThreads - 1
            Me.Thread(i) = New Thread(New ThreadStart(AddressOf Me.brute))
...
#End Region
    ...
    End Sub
    'https://www.google.ru/search?q=site:pastebin.com+@yahoo&num=100&tbs=qdr:D&ei=-d2PUqnoIOyp7AbP2IGoBQ&start=1&sa=N&biw=1366&bih=604
End Class
